...
-- Security policies (defensive – Edge Functions should use service role)
-- No SELECT policies (frontend can't read directly). Admins with service role bypass RLS.

CREATE POLICY "allow_insert_participants" ON public.participants
  FOR INSERT TO anon, authenticated
  WITH CHECK (true);

CREATE POLICY "allow_insert_background_survey_with_valid_participant" ON public.background_survey
  FOR INSERT TO anon, authenticated
  WITH CHECK (EXISTS (
    SELECT 1 FROM public.participants p WHERE p.participant_id = background_survey.participant_id
  ));

CREATE POLICY "allow_insert_task_instruction_with_valid_participant" ON public.task_instruction
  FOR INSERT TO anon, authenticated
  WITH CHECK (EXISTS (
    SELECT 1 FROM public.participants p WHERE p.participant_id = task_instruction.participant_id
  ));

CREATE POLICY "allow_insert_search_sessions_with_valid_participant" ON public.search_sessions
  FOR INSERT TO anon, authenticated
  WITH CHECK (EXISTS (
    SELECT 1 FROM public.participants p WHERE p.participant_id = search_sessions.participant_id
  ));

CREATE POLICY "allow_insert_queries_if_parent_session_exists" ON public.queries
  FOR INSERT TO anon, authenticated
  WITH CHECK (EXISTS (
    SELECT 1 FROM public.search_sessions s WHERE s.id = queries.session_id
  ));

CREATE POLICY "allow_insert_query_clicks_if_parent_query_exists" ON public.query_clicks
  FOR INSERT TO anon, authenticated
  WITH CHECK (EXISTS (
    SELECT 1 FROM public.queries q WHERE q.id = query_clicks.query_id
  ));

CREATE POLICY "allow_insert_scroll_events_if_parent_query_exists" ON public.scroll_events
  FOR INSERT TO anon, authenticated
  WITH CHECK (EXISTS (
    SELECT 1 FROM public.queries q WHERE q.id = scroll_events.query_id
  ));

CREATE POLICY "allow_insert_post_task_survey_with_valid_participant" ON public.post_task_survey
  FOR INSERT TO anon, authenticated
  WITH CHECK (EXISTS (
