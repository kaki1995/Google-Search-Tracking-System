        ...
        {currentResults.length > 0 && (
          <div className="w-full max-w-[700px] mx-auto mb-8">
            {currentResults.map((result) => (
              <div key={result.rank} className="py-3 border-b border-gray-200 last:border-b-0">
                <div className="mb-1">
                  <a 
                    href="#"
                    onClick={(e) => {
              ...
              {searchHistory.map((search, index) => (
                <div key={index} className="flex items-center justify-between p-3 bg-muted rounded-lg">
                  ...
                  <Button 
                    variant="outline" 
                    size="sm" 
                    onClick={() => handleCopyLink(`https://www.google.com/search?q=${encodeURIComponent(search.query)}`)} 
                    className="ml-4"
                  >
